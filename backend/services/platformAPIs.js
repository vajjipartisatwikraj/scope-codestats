...
class PlatformAPI {
  ...
  calculateLeetCodeScore(
    ...
    const problemsScore = totalSolved * 5;

    // Rating component - only considered if participated in at least 3 contests and rating > 1300
    let ratingScore = 0;
    if (contestsParticipated >= 3 && rating > 1400) {
      ratingScore = Math.pow(rating - 1400, 2) / 30;
    }
  ...
  calculateCodeforcesScore(rating, problemsSolved, contestsParticipated = 0) {
    ...
    // Problems solved component
    const problemsScore = problemsSolved * 5;

    // Rating component - only considered if participated in at least 3 contests and rating > 700
    let ratingScore = 0;
    if (contestsParticipated >= 3 && rating > 1000) {
      ratingScore = Math.pow(rating - 1000, 2) / 30;
    }
  ...
  calculateCodeChefScore(
    ...
    let ratingScore = 0;
    if (contestsParticipated >= 3 && rating > 1100) {
      ratingScore = Math.pow(rating - 1100, 2) / 30;
    }
  ...
  calculateHackerRankScore(data) {
    ...
    return problemsSolved * 5;
  }

  /**
   * Calculate normalized score for ScopeCodestats (internal platform)
   * @param {Object} data - ScopeCodestats profile data
   * @returns {number} - Calculated score
   */
  calculateScopeCodestatsScore(data) {
    // Formula: score = (total cohort scores) + (Consistency Index × 10)
    //
    // NOTE: Practice Arena contest count is intentionally NOT included in the score.
    // Practice Arena data is still fetched and stored for informational display,
    // but it does NOT contribute to the scopecodestats platform score or totalScore.

    const totalCohortScore = data.totalCohortScore || 0;
    const consistencyIndex = data.consistencyIndex || 0;

    const score = totalCohortScore + consistencyIndex * 10;

    return Math.round(score);
  }

  /**
   * Fetch ScopeCodestats profile (internal platform data)
   * @param {string} userId - User ID (not username, as this is internal)
   * @returns {Object} - User profile data from internal platform
   */
  async getScopeCodestatsProfile(userId) {
    try {
      console.log(`Fetching ScopeCodestats profile for user: ${userId}`);

      // Import models (require here to avoid circular dependency issues)
      const User = require("../models/User");
      const UserCohort = require("../models/UserCohort");
      const PATest = require("../models/PATest");

      // Get user's consistency index and lifetime score
      const user = await User.findById(userId)
        .select(
          "consistencyIndex lifetimeScopeScore lifetimeCohortProblemsSolved email rollNumber",
        )
        .lean();
      ...
      // Get cohort data - total problems solved and scores from ACTIVE cohorts
      const userCohorts = await UserCohort.find({
        user: userId,
        status: { $in: ["enrolled", "completed"] },
      }).lean();

      let totalCohortsCompleted = 0;
      let totalCohortProblemsolved = 0;
      let activeCohortScore = 0;

      userCohorts.forEach((userCohort) => {
        // Count completed cohorts
        if (userCohort.status === "completed") {
          totalCohortsCompleted++;
        }

        // Sum up problems solved from questionProgress
        if (
          userCohort.questionProgress &&
          Array.isArray(userCohort.questionProgress)
        ) {
          const solvedQuestions = userCohort.questionProgress.filter(
            (q) => q.solved === true,
          );
          totalCohortProblemsolved += solvedQuestions.length;
        }

        // Sum up total scores from ACTIVE cohorts only
        activeCohortScore += userCohort.totalScore || 0;
      });

      // Use MAXIMUM of lifetime score vs active cohort score
      // This ensures score NEVER decreases when cohorts are deleted
      const lifetimeScore = user.lifetimeScopeScore || 0;
      const totalCohortScore = Math.max(lifetimeScore, activeCohortScore);

      // Use MAXIMUM of lifetime problems vs active cohort problems
      // This ensures problems solved count NEVER decreases when cohorts are deleted
      const lifetimeProblems = user.lifetimeCohortProblemsSolved || 0;
      const totalCohortProblemsSolved = Math.max(
        lifetimeProblems,
        totalCohortProblemsolved,
      );

      // Get Practice Arena contest participation
      const practiceArenaTests = await PATest.find({
        user: userId,
        status: "completed",
      }).lean();

      const totalPracticeArenaContests = practiceArenaTests.length;

      // Prepare the data object
      const scopeCodestatsData = {
        username: user.email || user.rollNumber, // Use email or rollNumber as username
        totalCohortsCompleted,
        totalCohortProblemsolved: totalCohortProblemsSolved, // Use the maximum (preserved count)
        totalCohortScore, // Use the maximum (preserved score)
        lifetimeScore, // Preserved all-time score
        lifetimeProblems, // Preserved all-time problems count
        activeCohortScore, // Current active cohorts score (for debugging)
        activeCohortProblems: totalCohortProblemsolved, // Current active cohorts problems (for debugging)
        consistencyIndex: user.consistencyIndex || 0,
        consistencyScore: (user.consistencyIndex || 0) * 10, // Consistency score is index × 10
        totalPracticeArenaContests,
        lastUpdated: new Date(),
      };

      // Calculate score using preserved lifetime score
      const score = this.calculateScopeCodestatsScore(scopeCodestatsData);
      scopeCodestatsData.score = score;

      console.log(`ScopeCodestats data for user ${userId}:`, {
        activeCohortScore,
        lifetimeScore,
        totalCohortScore, // Max of both
        activeCohortProblems: totalCohortProblemsolved,
        lifetimeProblems,
        totalCohortProblemsSolved, // Max of both
        consistencyIndex: user.consistencyIndex,
        consistencyScore: scopeCodestatsData.consistencyScore,
        contestCount: totalPracticeArenaContests,
        calculatedScore: score,
      });

      return scopeCodestatsData;
    } catch (error) {
      console.error(
        `Error fetching ScopeCodestats profile for ${userId}:`,
        error,
      );
      throw new Error(
        `Failed to fetch ScopeCodestats profile: ${error.message}`,
      );
    }
  }
  ...
  async getLeetCodeProfile(username) {
    try {
      ...
      console.log(
        `Making GraphQL request to LeetCode API for user: ${username}`,
      );
      ...
      const response = await this.axiosInstance
        ...
        .catch((err) => {
          console.error(
            `LeetCode GraphQL main query failed with: ${err.message}`,
          );
          console.error(
            `Status code: ${err.response?.status}, Status text: ${err.response?.statusText}`,
          );
      ...
      if (
        ...
        console.warn(
          `Main profile query failed or returned invalid data for ${username}, will continue with partial data`,
        );
      ...
      if (userData.contestRating) {
        console.log(
          `Found contest data in main profile response for ${username}`,
        );
        contestsParticipated =
          userData.contestRating.attendedContestsCount || 0;
        ...
        console.warn(
          `No contest data found in main profile response for ${username}, trying contest query`,
        );

        // Try to get contest data separately
        try {
          console.log(
            `Attempting to fetch contest data for ${username} using dedicated query`,
          );
          const contestResponse = await this.axiosInstance
            .post(graphqlEndpoint, contestQuery, {
              headers: {
                "Content-Type": "application/json",
                Referer: `https://leetcode.com/${username}/`,
                "User-Agent":
          ...
          if (contestResponse.data) {
            try {
              console.log(
                `Contest API response:`,
                JSON.stringify(contestResponse.data, null, 2),
              );
            } catch (e) {
          ...
          if (contestResponse.data?.data?.userContestRanking) {
            ...
            console.warn(
              `No contest data found in dedicated contest query response`,
            );
          }
        } catch (contestError) {
        ...
        if (contestsParticipated === 0 && rating === 0) {
          try {
            console.log(
              `Trying alternative method for ${username} contest data using public API`,
            );

            // Use the public API endpoint that doesn't require authentication
            const publicApiUrl = `https://leetcode.com/graphql/?query=query%20userContestRankingInfo(%24username%3A%20String!)%20%7B%0A%20%20userContestRanking(username%3A%20%24username)%20%7B%0A%20%20%20%20attendedContestsCount%0A%20%20%20%20rating%0A%20%20%20%20globalRanking%0A%20%20%20%20totalParticipants%0A%20%20%20%20topPercentage%0A%20%20%20%20badge%20%7B%0A%20%20%20%20%20%20name%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D&variables=%7B%22username%22%3A%22${encodeURIComponent(
              username,
            )}%22%7D`;
            ...
            const contestResponse = await this.axiosInstance
              ...
              .catch((err) => {
                console.warn(
                  `Alternative contest data fetch failed: ${err.message}`,
                );
                if (err.response?.status) {
                  console.warn(
                    `Status code: ${err.response.status}, Status text: ${err.response.statusText}`,
            ...
            if (contestResponse.data) {
              try {
                console.log(
                  `Alternative contest API response:`,
                  JSON.stringify(contestResponse.data, null, 2),
                );
              } catch (e) {
                console.log(
                  "Contest response available but could not stringify",
                );
              }
            }
            ...
            console.warn(
              `Alternative contest data fetch failed with error: ${alternativeError.message}`,
            );
          }
          ...
          if (contestsParticipated === 0 && rating === 0) {
            try {
              console.log(
                `Trying to scrape contest data from ${username}'s profile page as last resort`,
              );

              const profilePageResponse = await this.axiosInstance
                .get(`https://leetcode.com/${username}/`, {
                  headers: {
                    "User-Agent":
                      "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
              ...
              if (profilePageResponse && profilePageResponse.status === 200) {
                console.log(
                  `Successfully fetched ${username}'s profile page, looking for contest data`,
                );
                ...
                if (contestsMatch && contestsMatch[1]) {
                  ...
                  console.log(
                    `Found contests participated from HTML: ${contestsParticipated}`,
                  );
                }
              ...
              console.warn(
                `Profile page scraping failed: ${scrapingError.message}`,
              );
            }
          }
        }
      ...
      if (allSolved === 0 && !userData.profile) {
        ...
        if (!userData.username && Object.keys(userData).length === 0) {
          ...
          console.error(
            `No user data found for "${username}" - likely a non-existent user`,
          );
          throw new Error(
            `User ${username} not found on LeetCode. Please check the spelling and try again.`,
          );
        }

        console.log(
          `Minimal data available for ${username}, creating basic profile`,
        );
        return {
          username,
          problemsSolved: 0,
      ...
      if (
        ...
        console.log(
          `LeetCode API authentication error (${error.response.status}) for ${username}, trying direct profile page request`,
        );
        ...
        try {
            ...
            .catch((err) => {
              ...
              if (err.response?.status) {
                ...
                if (err.response.status === 404) {
                  console.error(
                    `LeetCode profile page returned 404 Not Found for ${username}`,
                  );
                  throw new Error(
                    `User ${username} not found on LeetCode. Please check the username and try again.`,
                  );
                }
              }
              return null;
          ...
          if (!profilePageResponse) {
            console.error(
              `Could not verify existence of LeetCode user ${username}`,
            );
            throw new Error(
              `User ${username} not found on LeetCode. Please check the username and try again.`,
            );
          ...
          if (
            ...
            nonExistentIndicators.some((indicator) =>
              pageContent.includes(indicator),
            )
          ) {
            console.error(
              `LeetCode profile page indicates user ${username} doesn't exist`,
            );
            throw new Error(
              `User ${username} not found on LeetCode. Please check the username and try again.`,
            );
          ...
          if (
            profilePageResponse &&
            profilePageResponse.status === 200 &&
            hasProfileSection
          ) {
            console.log(
              `Successfully confirmed ${username} exists on LeetCode via profile page`,
            );
            ...
            if (totalMatch && totalMatch[1]) {
              ...
              console.log(
                `Found total problems solved: ${minimalProfile.problemsSolved}`,
              );
            }
            ...
            if (mediumMatch && mediumMatch[1]) {
              minimalProfile.mediumProblemsSolved = parseInt(
                mediumMatch[1],
                10,
              );
            }
            ...
            if (contestsMatch && contestsMatch[1]) {
              minimalProfile.contestsParticipated = parseInt(
                contestsMatch[1],
                10,
              );
              console.log(
                `Found contests participated: ${minimalProfile.contestsParticipated}`,
              );
            }
            ...
            if (minimalProfile.problemsSolved > 0) {
              ...
              console.log(
                `Calculated score for ${username} from HTML data: ${minimalProfile.score}`,
              );
            }
          ...
          console.error(
            `Final fallback HTML scraping failed: ${fallbackError.message}`,
          );

          // At this point, we couldn't verify the user exists, so we should not return a profile
          throw new Error(
            `Could not verify if user ${username} exists on LeetCode. Please check the username and try again.`,
          );
        }

        // If profile page check also fails, try a very simple HEAD request
        try {
          const headCheck = await this.axiosInstance.head(
            `https://leetcode.com/${username}/`,
          );
          if (headCheck.status === 200) {
            console.log(
              `Confirmed ${username} exists (HEAD request) but couldn't get profile data`,
            );
            minimalProfile.existsConfirmed = true;
            return minimalProfile;
        ...
        console.error(
          `Could not verify existence of LeetCode user ${username} after multiple attempts`,
        );
        throw new Error(
          `Could not verify if user ${username} exists on LeetCode. Please check the username and try again.`,
        );
      ...
      if (error.response?.status === 429) {
        ...
        throw new Error(
          `LeetCode API rate limit exceeded. Please try again in a few minutes.`,
        );
      }
  ...
  async getCodeforcesProfile(username) {
    try {
      ...
      if (this.codeforcesRequestCount >= 5) {
        ...
        if (remainingTime > 0) {
          console.log(
            `Rate limiting: Waiting ${remainingTime}ms before Codeforces request`,
          );
          await new Promise((resolve) =>
            setTimeout(resolve, Math.min(remainingTime, 5000)),
          );
        }
      }
      ...
      if (
        this.lastCodeforcesRequest &&
        now - this.lastCodeforcesRequest < 1000
      ) {
      ...
      if (userResponse.status === 429) {
        ...
        this.lastCodeforcesRequest = 0;
        throw new Error(
          `Codeforces API rate limit exceeded. Please try again in a few minutes.`,
        );
      }
      ...
      console.log(
        `Retrieved basic Codeforces data for ${username}: rating=${userData.rating}, rank=${userData.rank}`,
      );

      // 2. Get rating history to count contest participation
      const ratingResponse = await this.axiosInstance.get(
      ...
      if (ratingResponse.status === 429) {
        console.log(
          "Codeforces API rate limit hit for rating history (429 response)",
        );
      ...
      if (ratingResponse.data.status === "OK") {
        ...
        console.log(
          `${username} has participated in ${contestsParticipated} contests`,
        );
      }
      ...
      try {
        ...
        if (submissionsResponse.status === 429) {
          console.log(
            "Codeforces API rate limit hit for submissions (429 response)",
          );
        ...
        console.error(
          `Error fetching Codeforces submissions for ${username}:`,
      ...
      if (
        ...
        throw new Error(
          `User ${username} not found on Codeforces. Please check the spelling and try again.`,
        );
      }
      ...
      if (error.response?.status === 503) {
        throw new Error(
          `Codeforces API service is temporarily unavailable. Please try again later.`,
        );
      }
  ...
  processCodeforcesSubmissions(
    ...
    console.log(
      `${username} has solved ${problemsSolved} unique problems on Codeforces`,
    );
    console.log(
      `Problem difficulty breakdown - Easy: ${easyProblemsSolved}, Medium: ${mediumProblemsSolved}, Hard: ${hardProblemsSolved}`,
    );
  ...
  processCodeforcesData(
    ...
    if (
      problemsSolved === 0 &&
      contestsParticipated === 0 &&
      (userData.rating === undefined || userData.rating === 0)
    ) {
      console.log(
        `Codeforces user ${username} exists but has no activity data - returning zeros`,
      );
  ...
  async getCodeChefProfile(username) {
    try {
      ...
      if (this.lastCodeChefRequest && now - this.lastCodeChefRequest < 2000) {
        ...
        console.log(
          `Rate limiting: Waiting ${delay}ms before CodeChef request`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
      ...
      if (!profileContent) {
        console.log(
          `No profile content found for user ${username} on CodeChef`,
        );

        // Check if we're seeing a login page or error page
        const isLoginPage =
          $("body").text().includes("Login") &&
          $("body").text().includes("Forgot Password");
        const isErrorPage =
          $("body").text().includes("Oops") &&
          $("body").text().includes("Error");

        if (isLoginPage) {
          throw new Error(
            `Unable to access CodeChef profile for ${username}. The site may be requiring login.`,
          );
        } else if (isErrorPage) {
          throw new Error(
            `CodeChef returned an error for the profile ${username}. The user may not exist.`,
          );
        } else {
          throw new Error(
            `User ${username} not found on CodeChef or the profile format has changed`,
          );
        }
      }

      // Check if we see actual username on the page
      const usernameOnPage = $(
        "h1.h2-style, .user-details-container h1, .m-username, section.user-details h2",
      )
        .text()
        .trim();
      ...
      for (const selector of problemSelectors) {
        $(selector).each((_, el) => {
          ...
          if (
            elementText.includes("Fully Solved") ||
            elementText.includes("Problems Solved")
          ) {
            ...
            if (problemsMatch && problemsMatch[1]) {
              ...
              console.log(
                `Found problems solved (${problemsSolved}) in selector: ${selector}`,
              );
      ...
      if (problemsSolved === 0) {
        ...
        for (const pattern of problemPatterns) {
          ...
          if (matches && matches[1]) {
            ...
            console.log(
              `Found problems solved (${problemsSolved}) using text pattern`,
            );
            break;
          }
        }
      ...
      $("h5, .h5-style").each((_, el) => {
        ...
        if (
          text.includes("Contest Participated") ||
          text.includes("Contests Participated")
        ) {
          const next = $(el).next();
          if (next.length) {
      ...
      if (contestsParticipated === 0) {
        $(
          ".contest-participated-count, .rating-data-section strong, .user-details-container strong",
        ).each((_, el) => {
          const text = $(el).text().trim();
          ...
          if (
            (parentText.includes("contest") &&
              parentText.includes("participated")) ||
            ...
            if (matches) {
              ...
              console.log(
                `Found contest count in profile details: ${contestsParticipated}`,
              );
            }
          }
        });
      ...
      console.log(
        `Final CodeChef ${username} contests participated: ${contestsParticipated}`,
      );
      ...
      if (stars === 0) {
        const starText =
          $(".rating-header").text() || $(".user-details-container").text();
      ...
      if (
        ...
        throw new Error(
          `CodeChef request timed out. The service may be temporarily unavailable.`,
        );
      }
  ...
  async getGitHubProfile(username) {
    try {
      ...
      if (
        ...
        console.warn(
          "GitHub API token not configured. API calls will be limited to 60 requests/hour.",
        );
      } else {
      ...
      const authHeader =
        GITHUB_ACCESS_TOKEN && GITHUB_ACCESS_TOKEN !== "your_github_token_here"
      ...
      if (userResponse.status === 404) {
        console.log(
          `GitHub API confirms user ${username} does not exist (404 status)`,
        );
        throw new Error(
          `User "${username}" not found on GitHub. Please verify the username is correct and the profile is public.`,
        );
      }
      ...
      const rateLimit = {
        ...
        remaining: parseInt(
          userResponse.headers["x-ratelimit-remaining"] || "0",
        ),
      ...
      if (
        ...
        console.log(
          `Using GitHub GraphQL API to get contributions data for ${username}`,
        );
        ...
        try {
          ...
          if (
            ...
            graphqlResponse.data.data.user
          ) {
            const graphqlData = graphqlResponse.data.data.user;

            // Extract contributions
            if (
              graphqlData.contributionsCollection &&
              ...
              console.log(
                `GraphQL API: Found ${contributions} contributions for ${username}`,
              );
            }
            ...
            if (graphqlData.repositories && graphqlData.repositories.nodes) {
              ...
              console.log(
                `GraphQL API: Found ${totalStars} total stars for ${username}`,
              );
            }
          ...
          console.error(
            `Error using GitHub GraphQL API: ${graphqlError.message}`,
          );
        ...
        console.warn(
          "GitHub token not available - contribution data will be limited",
        );
      }
      ...
      return {
        ...
        starsReceived: totalStars || 0,
        problemsSolved: 0, // GitHub doesn't solve coding problems, always 0
        score,
        lastUpdated: new Date(),
      };
    } catch (error) {
      ...
      if (error.message.includes("Invalid GitHub username format")) {
        throw new Error(
          `Invalid GitHub username format: "${username}". GitHub usernames can only contain alphanumeric characters, hyphens and underscores.`,
        );
      }
  ...
  isValidGitHubUsername(username) {
    ...
    return /^[a-zA-Z0-9](?:[a-zA-Z0-9]|_|-(?=[a-zA-Z0-9])){0,38}$/.test(
      username,
    );
  }
  ...
  async getProfileData(platform, username) {
    try {
      ...
      console.log(
        `Getting profile data for ${normalizedPlatform}/${normalizedUsername}`,
      );
      ...
      switch (normalizedPlatform) {
        ...
        case "github":
          ...
          break;
        case "scopecodestats":
          // For scopecodestats, username is actually userId
          profileData = await this.getScopeCodestatsProfile(normalizedUsername);
          break;
        default:
      ...
      if (!profileData || typeof profileData !== "object") {
        console.error(
          `Invalid profile data returned for ${normalizedPlatform}/${normalizedUsername}`,
        );
        throw new Error(
          `Failed to retrieve valid profile data for ${normalizedUsername} on ${normalizedPlatform}`,
        );
      }
      ...
      console.error(
        `Error in getProfileData for ${platform}/${username}:`,
        error,
      );
      ...
      if (
        ...
        throw new Error(
          `${error.message}. Please check the spelling and try again.`,
        );
      }
