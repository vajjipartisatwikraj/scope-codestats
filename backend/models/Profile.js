 ...
 * When a User is deleted, all Profile documents with matching 'userId' field
 * are automatically deleted via cascade deletion middleware in User.js
 * 
 * Related deletion query: Profile.deleteMany({ userId: userId })
 */
...
const ProfileSchema = new mongoose.Schema({
  ...
  platform: {
    type: String,
    required: true,
    enum: ['leetcode', 'codeforces', 'codechef', 'geeksforgeeks', 'hackerrank', 'github', 'scopecodestats']
  },
  username: {
    type: String,
    required: true
...
ProfileSchema.methods.validateProfileFormat = function() {
  ...
  switch (this.platform) {
    ...
    case 'scopecodestats':
      // ScopeCodestats uses userId (MongoDB ObjectId) or email as username
      // Accept either valid ObjectId format or email format
      return /^[0-9a-fA-F]{24}$/.test(this.username) || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.username);
