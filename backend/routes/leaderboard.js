...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (debug === 'true') {
      console.log(`Leaderboard request - Page: ${pageNum}, Limit: ${limitNum}, Type: ${leaderboardType}`);
    }
        ...
        $addFields: {
          ...
          totalContestsParticipated: {
            $reduce: {
              input: '$profilesArray',
              initialValue: 0,
              in: { $add: ['$$value', { $ifNull: ['$$this.contestsParticipated', 0] }] }
            }
          },
          totalRating: {
            $reduce: {
    ...
    if (skipUserRank !== 'true' && req.user) {
      try {
        ...
        if (currentUserData) {
          ...
          if (department && department !== 'ALL' && currentUserData.department !== department) {
            userMatchesFilters = false;
          }
          if (section && section !== 'All' && currentUserData.section !== section) {
            userMatchesFilters = false;
          }
          if (graduatingYear && graduatingYear !== 'All' && currentUserData.graduatingYear !== parseInt(graduatingYear)) {
            userMatchesFilters = false;
          }
          if (search.trim()) {
            const searchLower = search.trim().toLowerCase();
            const nameMatch = currentUserData.name?.toLowerCase().includes(searchLower);
            const emailMatch = currentUserData.email?.toLowerCase().includes(searchLower);
            const rollMatch = currentUserData.rollNumber?.toLowerCase().includes(searchLower);
          ...
          if (userMatchesFilters) {
            ...
            const higherRankedCount = rankResult.length > 0 ? rankResult[0].higherRanked : 0;
      ...
      console.log('Skipping current user data fetch - no user or skipUserRank is true');
    }
...
router.get('/stats', auth, async (req, res) => {
  try {
    ...
    const [stats, platformStats] = await Promise.all([statsPromise, platformStatsPromise]);
...
router.get('/top', auth, async (req, res) => {
  try {
    ...
    const sortField = leaderboardType === 'score' ? 'totalScore' : 'problemsSolved';
