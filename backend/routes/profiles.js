...
router.post("/:platform", auth, async (req, res) => {
  try {
    ...
    // Check rate limiting
    const rateLimitCheck = await checkProfileUpdateRateLimit(
      req.user.id,
      platform,
    );
    ...
    try {
      ...
      try {
        ...
        platformData = await platformAPIService.getProfileData(
          platform,
          username,
        );
        ...
        if (platform === "codeforces") {
          ...
          if (
            platformData.rating === undefined &&
            platformData.rank === undefined &&
            platformData.problemsSolved === undefined &&
            platformData.score === undefined
          ) {
            throw new Error(
              `Codeforces user '${username}' not found or has no public data available`,
            );
          }
        }
      } catch (fetchError) {
        ...
        // For other platforms, rethrow
        throw fetchError;
      }

      if (!platformData || !platformData.username) {
        throw new Error(
          `Failed to fetch ${platform} profile data for user '${username}'`,
        );
      }
      ...
      if (platform === "github") {
        ...
        Object.assign(profileData, {
          ...
          globalRank:
            platform === "codechef"
              ? platformData.global_rank || 0
              : platform === "leetcode"
                ? platformData.ranking || 0
                : 0,
          countryRank:
            platform === "codechef" ? platformData.country_rank || 0 : 0,
          instituteRank: 0,
...
router.put("/update-scores", auth, async (req, res) => {
  try {
    ...
    const preserveExistingUsernames =
      req.body.preserveExistingUsernames === true;
    ...
    for (const profile of existingProfiles) {
      ...
      if (providedUsernames[profile.platform]) {
        ...
        if (newUsername && newUsername !== profile.username) {
          console.log(
            `Updating username for ${profile.platform} from ${profile.username} to ${newUsername}`,
          );
          profile.username = newUsername;
          await profile.save();
        } else if (!newUsername && preserveExistingUsernames) {
          console.log(
            `Keeping existing username for ${profile.platform}: ${profile.username}`,
          );
    ...
    for (const [platform, username] of Object.entries(providedUsernames)) {
      if (!existingPlatforms.includes(platform) && username.trim() !== "") {
        console.log(
          `Creating new profile for ${platform} with username ${username}`,
        );
    ...
    if (preserveExistingUsernames) {
      for (const [platform, username] of Object.entries(
        existingUsernamesByPlatform,
      )) {
        if (
          !existingPlatforms.includes(platform) &&
          !providedUsernames[platform] &&
          username
        ) {
          console.log(
            `Creating new profile for ${platform} with existing username ${username}`,
          );
          const newProfile = new Profile({
            userId: req.user.id,
            platform,
    ...
    const codechefProfile = profiles.find(
    ...
    if (codechefProfile) {
      try {
        ...
        console.log(
          `Using CodeChef problems solved (${codechefProblemsSolved}) as overall count`,
        );
      } catch (codechefError) {
        console.error(
          "Error fetching CodeChef profile:",
          codechefError.message,
        );
      }
    }
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            platformData = await Promise.race([
              platformAPIService.getProfileData(
                profile.platform,
                profile.username,
              ),
              new Promise((_, reject) =>
                setTimeout(() => reject(new Error("Timeout on retry")), 10000),
              ),
            ]);
          } else {
        ...
        if (profile.platform === "github") {
          userProfilesData[profile.platform].totalCommits =
            platformData.totalCommits || 0;
          userProfilesData[profile.platform].publicRepos =
            platformData.publicRepos || 0;
          userProfilesData[profile.platform].followers =
            platformData.followers || 0;
          userProfilesData[profile.platform].starsReceived =
            platformData.starsReceived || 0;
        } else if (
          ...
          userProfilesData[profile.platform].contestsParticipated =
            platformData.contestsParticipated || 0;
          userProfilesData[profile.platform].rating = platformData.rating || 0;
          userProfilesData[profile.platform].maxRating =
            platformData.maxRating || 0;

          // Add difficulty-wise problems for coding platforms
          if (platformData.easyProblemsSolved !== undefined) {
            userProfilesData[profile.platform].easyProblemsSolved =
              platformData.easyProblemsSolved || 0;
          }
          if (platformData.mediumProblemsSolved !== undefined) {
            userProfilesData[profile.platform].mediumProblemsSolved =
              platformData.mediumProblemsSolved || 0;
          }
          if (platformData.hardProblemsSolved !== undefined) {
            userProfilesData[profile.platform].hardProblemsSolved =
              platformData.hardProblemsSolved || 0;
          }
        }
    ...
    const userUpdateData = {
      ...
      // Contest statistics
      contestStats: {
        totalContestsParticipated: Object.values(userProfilesData).reduce(
          (total, platform) => total + (platform.contestsParticipated || 0),
          0,
        ),
        contestsByPlatform: {
          codeforces: userProfilesData.codeforces
            ? userProfilesData.codeforces.contestsParticipated || 0
            : 0,
          codechef: userProfilesData.codechef
            ? userProfilesData.codechef.contestsParticipated || 0
            : 0,
          leetcode: userProfilesData.leetcode
            ? userProfilesData.leetcode.contestsParticipated || 0
            : 0,
          hackerrank: userProfilesData.hackerrank
            ? userProfilesData.hackerrank.contestsParticipated || 0
            : 0,
        },
      },
      ...
      problemStats: {
        ...
        easySolved: Object.values(userProfilesData).reduce(
          (total, platform) => total + (platform.easyProblemsSolved || 0),
          0,
        ),
        mediumSolved: Object.values(userProfilesData).reduce(
          (total, platform) => total + (platform.mediumProblemsSolved || 0),
          0,
        ),
        hardSolved: Object.values(userProfilesData).reduce(
          (total, platform) => total + (platform.hardProblemsSolved || 0),
          0,
        ),
      },
    };
    ...
    console.log(
      `Profiles synced successfully for user ${req.user.id}. Total score: ${totalScore}`,
    );
...
router.put("/update-user/:userId", async (req, res) => {
  try {
    ...
    const platforms = [
      ...
      "hackerrank",
      "github",
      "scopecodestats", // Internal platform
    ];
    let totalScore = 0;
    let totalProblemsSolved = 0;
    const updatedProfiles = [];
    ...
    for (const platform of platforms) {
      // For scopecodestats, use userId instead of username
      const username =
      ...
      // Skip platforms where the user hasn't set a username (except scopecodestats)
      if (!username && platform !== "scopecodestats") {
      ...
      try {
        ...
        while (attempt < maxAttempts) {
          try {
            ...
            if (attempt > 0) {
              ...
              console.log(
                `Retry attempt ${attempt} for ${platform} profile of ${username}`,
              );
            }
              ...
              new Promise((_, reject) =>
                setTimeout(
                  () =>
                    reject(
                      new Error(
                        `${platform} API request timeout after ${timeoutMs}ms`,
                      ),
                    ),
                  timeoutMs,
                ),
        ...
        // For scopecodestats, problemsSolved should equal totalCohortProblemsolved
        if (platform === "scopecodestats") {
          profile.problemsSolved = platformData.totalCohortProblemsolved || 0;
        } else {
        ...
        } else if (platform === "scopecodestats") {
          // ScopeCodestats specific fields
          profile.details = {
            totalCohortsCompleted: platformData.totalCohortsCompleted || 0,
            totalCohortProblemsolved:
              platformData.totalCohortProblemsolved || 0,
            totalCohortScore: platformData.totalCohortScore || 0,
            consistencyIndex: platformData.consistencyIndex || 0,
            consistencyScore: platformData.consistencyScore || 0,
            totalPracticeArenaContests:
              platformData.totalPracticeArenaContests || 0,
          };
        }
        ...
        // Exclude GitHub from total problems solved (only coding platforms count)
        if (platform !== "github") {
          // For scopecodestats, use totalCohortProblemsolved instead of problemsSolved
          if (platform === "scopecodestats") {
            totalProblemsSolved += platformData.totalCohortProblemsolved || 0;
          } else {
        ...
        userProfilesData[platform] = {
          username: profile.username,
          score: profile.score,
          problemsSolved: platform === "github" ? 0 : profile.problemsSolved, // GitHub doesn't solve coding problems
          rating: profile.rating,
          rank: profile.rank,
          lastUpdated: profile.lastUpdated,
        };
        ...
        // Add platform-specific data to the user profile summary
        if (platform === "github") {
          userProfilesData[platform].totalCommits =
            platformData.totalCommits || 0;
          userProfilesData[platform].publicRepos =
            platformData.publicRepos || 0;
          userProfilesData[platform].followers = platformData.followers || 0;
          userProfilesData[platform].starsReceived =
            platformData.starsReceived || 0;
        } else if (
          platform === "codeforces" ||
          platform === "codechef" ||
          ...
          userProfilesData[platform].contestsParticipated =
            platformData.contestsParticipated || 0;
          ...
          // Add difficulty-wise problems for coding platforms
          if (platformData.easyProblemsSolved !== undefined) {
            userProfilesData[platform].easyProblemsSolved =
              platformData.easyProblemsSolved || 0;
          }
          if (platformData.mediumProblemsSolved !== undefined) {
            userProfilesData[platform].mediumProblemsSolved =
              platformData.mediumProblemsSolved || 0;
          }
          if (platformData.hardProblemsSolved !== undefined) {
            userProfilesData[platform].hardProblemsSolved =
              platformData.hardProblemsSolved || 0;
          }
        } else if (platform === "scopecodestats") {
          // Add ScopeCodestats specific data
          userProfilesData[platform].totalCohortsCompleted =
            platformData.totalCohortsCompleted || 0;
          userProfilesData[platform].totalCohortProblemsolved =
            platformData.totalCohortProblemsolved || 0;
          userProfilesData[platform].totalCohortScore =
            platformData.totalCohortScore || 0;
          userProfilesData[platform].consistencyIndex =
            platformData.consistencyIndex || 0;
          userProfilesData[platform].consistencyScore =
            platformData.consistencyScore || 0;
          userProfilesData[platform].totalPracticeArenaContests =
            platformData.totalPracticeArenaContests || 0;
        }
        ...
        // IMPORTANT: Only preserve last known good data if the USERNAME has NOT changed.
        //
        // WHY: If we always preserve on API failure, a user could exploit this by:
        //   1. Setting a high-scorer's username → sync → get their score
        //   2. Changing to a non-existent username → next sync fails
        //   3. Old stolen score preserved forever!
        //
        // FIX: Compare the CURRENT username (from user.profiles) with the username
        // stored in the existing platformScores data. If they differ, the user
        // changed their profile — DON'T preserve the old data (it belongs to
        // a different identity). Only preserve on genuine API outages where
        // the same username just temporarily failed to fetch.
        const existingPlatformData =
          user.platformScores instanceof Map
            ? user.platformScores.get(platform)
            : user.platformScores && user.platformScores[platform];

        if (existingPlatformData && existingPlatformData.score > 0) {
          // Check if the username has changed from what's stored
          const storedUsername = existingPlatformData.username;
          const currentUsername = username; // The username we just tried to fetch

          if (
            platform === "scopecodestats" ||
            storedUsername === currentUsername
          ) {
            // Same username — genuine API failure, preserve the data
            console.log(
              `⚠️ Platform ${platform} failed for ${user.name} (same username: ${currentUsername}), preserving last known score: ${existingPlatformData.score}`,
            );
            userProfilesData[platform] = {
              ...existingPlatformData,
              lastUpdateStatus: "preserved",
              lastUpdateError: error.message,
              lastPreservedAt: new Date(),
            };
            // Add preserved score/problems to totals
            totalScore += existingPlatformData.score || 0;
            if (platform !== "github") {
              totalProblemsSolved += existingPlatformData.problemsSolved || 0;
            }
          } else {
            // Username CHANGED — the old data is from a different profile.
            // Do NOT preserve the old score. Zero it out.
            console.log(
              `🚫 Platform ${platform} failed for ${user.name} — username changed from "${storedUsername}" to "${currentUsername}". ` +
                `NOT preserving old score (${existingPlatformData.score}). This prevents username swap exploits.`,
            );
            userProfilesData[platform] = {
              ...
              rating: 0,
              rank: "unrated",
              lastUpdateStatus: "username_changed_failed",
              lastUpdateError: `Username changed and new profile "${currentUsername}" failed to fetch: ${error.message}`,
              lastPreservedAt: new Date(),
            };
            // Score is 0 — do NOT add to totals
          }
        }
        ...
        updatedProfiles.push({
          ...
          preserved: !!(
            existingPlatformData &&
            existingPlatformData.score > 0 &&
            (platform === "scopecodestats" ||
              existingPlatformData.username === username)
          ),
        });
      }
    ...
    // Update user document with consolidated data
    // NOTE: totalScore will be recalculated by scoreAggregator.updateUserTotalScore() below.
    // totalProblemsSolved uses LIVE data - cohort problem counts are preserved
    // separately via user.lifetimeCohortProblemsSolved in scopecodestats platform.
    const userUpdateData = {
      totalScore, // Will be recalculated by scoreAggregator with live scores
      totalProblemsSolved, // Live count from all successfully fetched platforms
      lastProfileSync: new Date(),
      platformScores: userProfilesData,
      platformData: userProfilesData, // Also update platformData with the same values
      // Add overall GitHub metrics to user document for easy access
      githubStats: userProfilesData.github
        ? {
            totalCommits: userProfilesData.github.totalCommits || 0,
    ...
    // Update lifetime scope score and cohort problems if current active values exceed them
    // This ensures lifetimeScopeScore stays in sync during profile sync, not just during submissions
    if (userProfilesData.scopecodestats) {
      const scopeData = userProfilesData.scopecodestats;
      const lifetimeUpdates = {};

      // Check if active cohort score exceeds lifetime
      if (
        scopeData.totalCohortScore &&
        scopeData.totalCohortScore > (updatedUser.lifetimeScopeScore || 0)
      ) {
        lifetimeUpdates.lifetimeScopeScore = scopeData.totalCohortScore;
        console.log(
          `Updated lifetimeScopeScore: ${updatedUser.lifetimeScopeScore || 0} → ${scopeData.totalCohortScore}`,
        );
      }

      // Check if active cohort problems exceed lifetime
      if (
        scopeData.totalCohortProblemsolved &&
        scopeData.totalCohortProblemsolved >
          (updatedUser.lifetimeCohortProblemsSolved || 0)
      ) {
        lifetimeUpdates.lifetimeCohortProblemsSolved =
          scopeData.totalCohortProblemsolved;
        console.log(
          `Updated lifetimeCohortProblemsSolved: ${updatedUser.lifetimeCohortProblemsSolved || 0} → ${scopeData.totalCohortProblemsolved}`,
        );
      }

      if (Object.keys(lifetimeUpdates).length > 0) {
        await User.findByIdAndUpdate(userId, lifetimeUpdates);
      }
    }

    // Recalculate total score from updated platformScores to ensure accuracy
    ...
    console.log(
      `Profiles synced successfully for user ${userId}. Total score: ${totalScore}`,
    );
...
router.get("/me", auth, async (req, res) => {
  try {
    ...
    console.log(
      `Profile /me endpoint: User ${user._id}, rollNumber: ${user.rollNumber}, graduatingYear from DB: ${user.graduatingYear}, calculated/returned: ${graduationYear}`,
    );

    // Get all platform profiles
    const platformProfiles = await Profile.find({ userId: req.user.id }).lean();
      ...
      Object.keys(user.profiles).forEach((platform) => {
        if (user.profiles[platform] && !profile.profiles[platform]?.username) {
          profile.profiles[platform] = {
            username:
              typeof user.profiles[platform] === "object"
                ? user.profiles[platform].username || ""
...
router.put("/me", auth, async (req, res) => {
  try {
    ...
    if (graduationYear) {
      console.log(
        `Received explicit graduationYear: ${graduationYear}, setting graduatingYear field`,
      );
      ...
      if (rollNumber && rollNumber.length >= 2) {
        ...
        console.log(
          `No graduationYear provided. Calculated ${calculatedYear} from roll number: ${rollNumber}`,
        );
    ...
    // Handle platform profiles if provided
    if (profiles) {
      if (profiles.leetcode)
        updateFields["profiles.leetcode"] = profiles.leetcode;
      if (profiles.codeforces)
        updateFields["profiles.codeforces"] = profiles.codeforces;
      if (profiles.codechef)
        updateFields["profiles.codechef"] = profiles.codechef;
      if (profiles.hackerrank)
        updateFields["profiles.hackerrank"] = profiles.hackerrank;
      if (profiles.github) updateFields["profiles.github"] = profiles.github;
    }
    ...
    try {
      ...
      if (profiles || githubUrl) {
        const platformsToUpdate = [
          {
            key: "codechef",
            username: profiles?.codechef || user.profiles?.codechef || "",
          },
          {
            key: "codeforces",
            username: profiles?.codeforces || user.profiles?.codeforces || "",
          },
          {
            key: "leetcode",
            username: profiles?.leetcode || user.profiles?.leetcode || "",
          },
          {
            key: "hackerrank",
            username: profiles?.hackerrank || user.profiles?.hackerrank || "",
          },
          {
            key: "github",
            username:
              profiles?.github || githubUrl || user.profiles?.github || "",
          },
        ];
          ...
          if (platform.username && platform.username.trim() !== "") {
            try {
              ...
              if (profile) {
                ...
                await profile.save();
                console.log(
                  `Updated ${platform.key} profile for user ${user._id}`,
                );
              } else {
                ...
                await profile.save();
                console.log(
                  `Created new ${platform.key} profile for user ${user._id}`,
                );
              }
            } catch (profileError) {
              console.error(
                `Error creating/updating ${platform.key} profile:`,
                profileError,
              );
      ...
      console.log(
        `- Calculated graduation year: ${calculateGraduationYear(
          user.rollNumber,
        )}`,
      );
...
router.get("/platform/:platform/:username", async (req, res) => {
  try {
    ...
    const platformData = await platformAPIService.getProfileByPlatform(
      platform,
      username,
    );
...
router.get("/debug-codeforces/:username", auth, async (req, res) => {
  try {
    ...
    if (forcePuppeteer) {
      ...
      profileData =
        await platformAPIService.getCodeforcesProfileWithPuppeteer(username);
    } else {
...
router.get("/sync-info", auth, async (req, res) => {
  try {
    ...
    const syncInfo = await mongoose.connection.db
      .collection("system")
    ...
    if (!syncInfo) {
      ...
      return res.json({
        ...
        message:
          "No sync has occurred yet. First sync will run at the scheduled time.",
      });
    }
    ...
    res.json({
      ...
      message: syncInfo.lastSync
        ? `Last sync completed on ${new Date(
            syncInfo.lastSync,
          ).toLocaleString()}. Next sync at ${new Date(
            syncInfo.nextSync,
          ).toLocaleString()}.`
...
router.get("/quick-sync", auth, async (req, res) => {
  try {
    ...
    console.log(
      `Starting quick sync for user ${userId} with ${profiles.length} profiles`,
    );
      ...
      try {
        ...
        for (const profile of profiles) {
          try {
            ...
            const platformData = await platformAPIService.getProfileData(
              platform,
              username,
            );
            ...
            console.error(
              `Error syncing ${profile.platform}/${profile.username}:`,
              profileError,
            );
        ...
        console.log(
          `Background sync completed for user ${userId}. Updated ${updatedProfiles.length}/${profiles.length} profiles.`,
        );
      } catch (syncError) {
  ...
  try {
    ...
    profileDocs.forEach((profile) => {
      if (platforms.includes(profile.platform)) {
        // Ensure username is a string
        result[profile.platform].username =
...
router.get("/stats", auth, async (req, res) => {
  try {
    ...
    const totalScore = profiles.reduce(
      (sum, profile) => sum + (profile.score || 0),
      0,
    );
...
// Get rank history for the current user (last 7 days by default)
router.get("/rank-history", auth, async (req, res) => {
  try {
    ...
    // Fetch rank history for the user
    const rankHistory = await RankHistory.getLastNDaysRanks(userId, daysNumber);

    if (!rankHistory || rankHistory.length === 0) {
      return res.json({
        success: true,
        message: "No rank history found",
        data: {
          userId: userId,
          rankHistory: [],
          summary: {
    ...
    const ranks = rankHistory.map((r) => r.overallRank);
    ...
    // Calculate trend (comparing first and last rank)
    ...
    if (ranks.length >= 2) {
      ...
      if (lastRank < firstRank) {
        rankTrend = "improving"; // Lower rank number = better position
      } else if (lastRank > firstRank) {
        rankTrend = "declining"; // Higher rank number = worse position
      }
    }

    // Format the data for frontend consumption (SparkLineChart expects numerical data)
    const chartData = rankHistory.map((record) => ({
      date: record.date.toISOString().split("T")[0], // YYYY-MM-DD format
      overallRank: record.overallRank,
      departmentRank: record.departmentRank,
      totalScore: record.totalScore,
      totalProblemsSolved: record.totalProblemsSolved,
      // For tooltip display
      dateFormatted: record.date.toLocaleDateString("en-US", {
        weekday: "short",
        month: "short",
        day: "numeric",
      }),
    ...
    res.json({
      ...
      data: {
        userId: userId,
        rankHistory: chartData,
        summary: {
          totalDays: rankHistory.length,
        ...
        // Additional data for SparkLineChart
        sparklineData: {
          // Invert ranks for better visualization (higher values = better)
          // Since rank 1 is best, we'll use (maxRank - rank + 1) for visualization
          ranks: ranks.map((rank) => {
            const maxRank = Math.max(...ranks);
            return maxRank - rank + 1; // Invert so higher is better for chart
          }),
          dates: chartData.map((d) => d.dateFormatted),
          originalRanks: ranks, // Keep original ranks for tooltip
        },
      },
    });
  } catch (err) {
    ...
    res.status(500).json({
      success: false,
      message: "Server error while fetching rank history",
      error: err.message,
    });
  }
});
...
router.get("/analytics", auth, async (req, res) => {
  try {
    ...
    const analyticsData = await RankHistory.getLastNDaysRanks(
      userId,
      daysNumber,
    );
    ...
    // Process platform trends (last 7 days for sparklines)
    ...
    platforms.forEach((platform) => {
      platformTrends[platform] = {
        scores: analyticsData.map(
          (d) => d.platformData?.[platform]?.score || 0,
        ),
        changes: analyticsData.map(
          (d) => d.platformData?.[platform]?.scoreChange || 0,
        ),
        dates: analyticsData.map((d) =>
    ...
    // Calculate score distribution (latest data)
    const latestData = analyticsData[analyticsData.length - 1];
    const scoreDistribution = {};
    let totalScore = 0;

    platforms.forEach((platform) => {
      const score = latestData.platformData?.[platform]?.score || 0;
      scoreDistribution[platform] = score;
      totalScore += score;
    });

    // Convert to percentages
    Object.keys(scoreDistribution).forEach((platform) => {
      scoreDistribution[platform] =
        totalScore > 0
          ? Math.round((scoreDistribution[platform] / totalScore) * 100)
          : 0;
    });

    // Calculate problems solved by platform (latest data)
    const problemsSolvedByPlatform = {};
    platforms.forEach((platform) => {
      problemsSolvedByPlatform[platform] =
        latestData.platformData?.[platform]?.problemsSolved || 0;
    });

    // Prepare daily activity heatmap data - ensure we have a full range
    const dailyActivityMap = {};
    analyticsData.forEach((record) => {
      dailyActivityMap[record.date.toISOString().split("T")[0]] = {
        date: record.date.toISOString().split("T")[0],
        score: record.totalScore,
        scoreChange: record.totalScoreChange || 0,
        problemsSolved: record.totalProblemsSolved,
        rank: record.overallRank,
        intensity: Math.min(Math.max(record.totalScoreChange || 0, 0), 100), // 0-100 scale for heatmap
      };
    });

    // Generate full range of days for the requested period
    const dailyActivity = [];
    ...
    for (
      ...
      const existingData = dailyActivityMap[dateString];

      if (existingData) {
        dailyActivity.push(existingData);
      } else {
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          currentScore: latestData.totalScore,
          currentRank: latestData.overallRank,
          totalScoreChange: analyticsData.reduce(
            (sum, d) => sum + (d.totalScoreChange || 0),
            0,
          ),
          totalProblemsChange: analyticsData.reduce(
            (sum, d) => sum + (d.totalProblemsChange || 0),
            0,
          ),
        },
      },
