...
router.post("/admin/user/:userId", auth, adminAuth, async (req, res) => {
  try {
    ...
    // ✨ WEBSOCKET: Emit real-time notification to user
    const io = req.app.get("io");
    if (io) {
      // Check if user is connected
      const userRoom = `user:${userId}`;
      const socketsInRoom = io.sockets.adapter.rooms.get(userRoom);
      const isUserOnline = socketsInRoom && socketsInRoom.size > 0;
      ...
      if (isUserOnline) {
        io.to(userRoom).emit("notification:new", {
          ...
          deletionTime: notification.deletionTime,
        });
        console.log(
          `[Socket.io] ✅ Sent notification to ONLINE user:${userId} (${socketsInRoom.size} connection(s))`
        );
      } else {
        console.log(
          `[Socket.io] ⚠️  User OFFLINE: ${userId} - notification saved, will receive on next login or via VAPID`
        );
      }
    }

    // Send push notification if user has push subscriptions (VAPID - for background notifications)
    if (user.pushSubscriptions && user.pushSubscriptions.length > 0) {
      try {
        ...
        // Send to all valid subscriptions, remove expired ones
        ...
        for (const subscription of user.pushSubscriptions) {
          try {
            const result = await webPushUtil.sendPushNotification(
              {
            ...
            if (result.status) {
              ...
              vapidExpired++;
              // Don't add to validSubscriptions - will be removed
            } else {
              vapidErrors++;
              validSubscriptions.push(subscription); // Keep for retry
            }
          } catch (error) {
            // Unexpected error - keep subscription
            console.error("[VAPID] Unexpected error:", error.message);
            validSubscriptions.push(subscription);
            vapidErrors++;
          }
        }
        ...
        if (validSubscriptions.length < user.pushSubscriptions.length) {
          const removedCount =
            user.pushSubscriptions.length - validSubscriptions.length;
          user.pushSubscriptions = validSubscriptions;
          await user.save();
          console.log(
            `[VAPID] 🧹 Cleaned up ${removedCount} expired subscription(s) for user:${userId}`
          );
        }
        ...
        if (vapidSent > 0) {
          console.log(
            `[VAPID] ✅ Sent push notification to ${vapidSent} active device(s) for user:${userId}`
          );
        }
        if (vapidExpired > 0 && vapidSent === 0) {
          console.log(
            `[VAPID] ⚠️  No active VAPID subscriptions for user:${userId} - all ${vapidExpired} subscription(s) expired`
          );
        }
      } catch (vapidError) {
        // Catch any unexpected VAPID errors to prevent notification creation from failing
        console.error(
          "[VAPID] Unexpected error in push notification handler:",
          vapidError.message
        );
      }
    } else {
      console.log(
        `[VAPID] ℹ️  User ${userId} has no push subscriptions registered`
      );
    }
...
router.post("/admin/global", auth, adminAuth, async (req, res) => {
  try {
    ...
    // ✨ WEBSOCKET: Emit real-time notification to all connected users
    const io = req.app.get("io");
    if (io && notifications.length > 0) {
      // Get first notification as template (all have same content)
      ...
      console.log(
        `[Socket.io] Sent global notification to ${notifications.length} users`
      );
    }
...
router.put("/admin/:id", auth, adminAuth, async (req, res) => {
  try {
    ...
    if (!title && !message && deletionTime === undefined) {
      return res
        .status(400)
        .json({ message: "Title, message, or deletion time is required" });
    }
...
const cleanupExpiredNotifications = async () => {
  try {
    ...
    if (result.deletedCount > 0) {
      console.log(
        `Auto-cleanup: Deleted ${result.deletedCount} expired notifications`
      );
    }
  } catch (error) {
...
/**
 * ⚠️ AUTO-CLEANUP DISABLED ⚠️
 * 
 * Automatic cleanup of expired notifications is DISABLED.
 * 
 * WHAT THIS DOES:
 * - Runs every hour to delete notifications older than 7 days
 * - Helps keep the database clean and performant
 * 
 * TO RE-ENABLE:
 * - Uncomment the setInterval line below
 * - Uncomment the initial cleanup call
 * 
 * MANUAL CLEANUP:
 * - Call cleanupExpiredNotifications() function directly
 * - Or create an admin API endpoint to trigger cleanup
 */

// ⚠️ DISABLED: Run cleanup every hour
// Uncomment the line below to enable automatic hourly cleanup
// setInterval(cleanupExpiredNotifications, 60 * 60 * 1000);

// ⚠️ DISABLED: Run an initial cleanup when the server starts
// Uncomment the line below to enable cleanup on server startup
// cleanupExpiredNotifications();

console.log('⚠️  Notification auto-cleanup is DISABLED');
console.log('💡 To enable: Uncomment setInterval and cleanupExpiredNotifications() calls in notifications.js');
...
router.post("/push/subscribe", auth, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Push notification subscription saved successfully" });
  } catch (error) {
