...
const CourseManagement = () => {
  ...
  // LAZY LOAD: Fetch courses only on first mount
  useEffect(() => {
    if (!hasLoadedData) {
      fetchCourses();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCourse) {
        ...
        await axios.post(`${apiUrl}/admin/courses`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
            ...
            <Grid item xs={12} sm={6} md={4} key={course._id}>
              ...
              >
                <CardMedia
                  ...
                  height="160"
                  image={
                    course.image || "https://source.unsplash.com/random?coding"
                  }
                  alt={course.title}
                  sx={{ objectFit: "cover" }}
                />
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <Autocomplete
                  ...
                  onChange={(e, newValue) =>
                    setFormData({ ...formData, topics: newValue })
                  }
                  options={[]}
                  renderInput={(params) => (
                    <TextField
