...
const QuestionForm = ({
  ...
  const handleTabChange = useCallback(
    (event, newValue) => {
      const maxTabIndex = formData.type === "mcq" ? 4 : 5; // Adjusted for new tab structure
  ...
  // Example management
  const handleExampleChange = useCallback((field, value) => {
  ...
  const getTabCount = () => {
    return formData.type === "mcq" ? 5 : 6; // Basic, Options/Languages, TestCases(prog only), Additional, Upload, Search
  };
