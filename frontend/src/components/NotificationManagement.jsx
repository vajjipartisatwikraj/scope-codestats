...
const NotificationManagement = () => {
  ...
  // Additional security check - ensure only admins and teachers can access this page
  useEffect(() => {
  ...
  const handleNotificationSubmit = async () => {
    try {
      const { title, message, targetType, selectedUser, deletionTime } =
      ...
      toast.success(
        editNotificationId ? "Notification updated" : "Notification created"
      );
      fetchNotifications();
  ...
  const handleEditNotification = (notification) => {
    setNotificationFormData({
      ...
      deletionTime: notification.deletionTime
        ? new Date(notification.deletionTime).toISOString().slice(0, 16)
        : "",
    });
  ...
  const handleRunCleanup = async () => {
    try {
      ...
      toast.success(
        `Cleanup complete: Deleted ${response.data.deletedCount} expired notifications`
      );
      fetchNotifications();
    } catch (error) {
  ...
  // LAZY LOAD: Fetch notifications and users only on first mount
  useEffect(() => {
    if (!hasLoadedData) {
      fetchNotifications();
      fetchUsers();
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={8}>
              ...
              <Typography variant="body1" sx={{ color: "white", opacity: 0.9 }}>
                Create and manage notifications for your users. Send targeted
        ...
        <Grid container spacing={3} sx={{ mb: 3 }}>
          <Grid item xs={12} sm={6} md={3}>
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
                <Avatar
                  sx={{ bgcolor: "rgba(0, 136, 204, 0.1)", color: "#0088cc" }}
                >
          ...
          <Grid item xs={12} sm={6} md={3}>
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
                <Avatar
                  sx={{ bgcolor: "rgba(103, 58, 183, 0.1)", color: "#673AB7" }}
                >
                  <PersonIcon />
                </Avatar>
              </Box>
                      ...
                      <TableCell>
                        <Typography
                          ...
                          sx={{
                            ...
                            alignItems: "center",
                            gap: 0.5,
                            color: notification.deletionTime
                              ? "warning.main"
                              : "#cccccc",
                          }}
                        >
                          {notification.deletionTime && (
                  ...
                  setNotificationFormData({
                    ...
                    selectedUser:
                      e.target.value === "global"
                        ? ""
                        : notificationFormData.selectedUser,
                  })
                }
              >
            ...
            <FormControl fullWidth margin="normal">
              ...
              >
                ...
                <Tooltip title="Set a time when this notification will be automatically deleted. Leave empty for no auto-deletion.">
                  <HelpOutlineIcon
                    fontSize="small"
                    sx={{ ml: 1, color: "text.secondary", fontSize: "16px" }}
                  />
                </Tooltip>
              </Typography>
              <TextField
              ...
              />
              <FormHelperText>
                Leave empty if you don't want the notification to be
