...
export const AuthProvider = ({ children }) => {
  ...
  // Use hooks with error boundary
  let navigate, location;
  try {
    navigate = useNavigate();
    location = useLocation();
  } catch (error) {
    // Hooks might fail during hot reload or navigation transitions
    console.warn(
      "[AuthContext] Router hooks temporarily unavailable:",
      error.message
    );
  ...
  useEffect(() => {
    // Safety check: Prevent execution if router is not ready
    if (!location || !navigate || typeof navigate !== "function") {
      return;
    }
    ...
    const handleOAuthRedirect = async () => {
      ...
      try {
        ...
        userData = {
          ...
          graduatingYear: fullUserData.graduatingYear,
          rollNumber: fullUserData.rollNumber,
  ...
  useEffect(() => {
    ...
    // If we have auth and we're on login/register/root pages, redirect to appropriate dashboard
    if (
      token &&
      (location.pathname === "/login" ||
        location.pathname === "/register" ||
    ...
    // Check if a non-admin/teacher is trying to access admin routes
    if (
      token &&
    ...
    // Check if an admin/teacher is at regular dashboard (not admin routes) and redirect to admin
    if (
      token &&
